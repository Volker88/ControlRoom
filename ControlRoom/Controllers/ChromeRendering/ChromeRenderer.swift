...
class ChromeRenderer { // swiftlint:disable:this type_body_length
    ...
    init(deviceName: String, screenshot: NSImage) throws {
        ...
        let basePath =
            "\(developerPath)/Platforms/iPhoneOS.platform/Library/Developer/CoreSimulator/Profiles"
        let profilePath =
            "\(basePath)/DeviceTypes/\(deviceName).simdevicetype/Contents/Resources/profile.plist"
        let profileURL = URL(filePath: profilePath)

        let profileData = try Data(contentsOf: profileURL)
        device = try PropertyListDecoder().decode(
        ...
        let mainIdentifier =
            device.chromeIdentifier.components(separatedBy: ".").last ?? "phone"

        // Now use that last part to find the PDFs and placement JSON.
        var chromePath =
            "\(basePath)/Chrome/\(mainIdentifier).devicechrome/Contents/Resources"
        if !FileManager.default.fileExists(atPath: chromePath) {
            ...
            chromePath =
                "\(basePath)/Chrome/\(mainIdentifier).simdevicechrome/Contents/Resources"
        }
        ...
        chrome = try JSONDecoder().decode(
            SimulatorChrome.self,
            from: chromeData
        )

        // Add just a little extra space for padding
        width =
            (Double(screenshot.size.width) / device.mainScreenScale + chrome
                .images.sizing.leftWidth * 2) * 1.05
        height =
            (Double(screenshot.size.height) / Double(device.mainScreenScale)
                + chrome.images.sizing.topHeight * 2) * 1.05
    }
            ...
            content:
                Canvas { [self] context, size in
                    ...
                    let screenshotSize = CGSize(
                        width: screenshot.size.width / device.mainScreenScale,
                        height: screenshot.size.height / device.mainScreenScale
                    )
                    ...
                    let topLeft = CGPoint(
                        x: size.width / 2 - (screenshotSize.width / 2),
                        y: size.height / 2 - (screenshotSize.height / 2)
                    )
                    ...
                    let drawHeight: Double =
                        bottomLeft.y - t.size.height - b.size.height - topLeft.y
                        + edgeSizes.top + edgeSizes.bottom + 2.0
                    let drawWidth: Double =
                        screenshotSize.width - tl.size.width - tr.size.width
                        + edgeSizes.left + edgeSizes.right + 2.0

                    // Now draw the inputs (i.e. buttons) that must be placed behind the rest of the
                    // chrome, such as the volume buttons.
                    let behindInputs = chrome.inputs.filter {
                        $0.onTop == false
                    }
                    draw(
                        inputs: behindInputs,
                    ...
                    let topX: Double =
                        size.width / 2.0 - (screenshotSize.width / 2.0)
                        + tl.size.width - edgeSizes.left - 1
                    ...
                    let bottomX: Double =
                        size.width / 2 - (screenshotSize.width / 2.0)
                        + tl.size.width - edgeSizes.left - 1
                    let bottomY: Double =
                        bottomLeft.y - b.size.height + edgeSizes.bottom
                    context.draw(
                        Image(nsImage: b),
                        in: CGRect(
                            x: bottomX,
                    ...
                    context.draw(
                        Image(nsImage: l),
                        in: CGRect(
                            x: topLeft.x - edgeSizes.left,
                            y: topLeft.y + tl.size.height - edgeSizes.top - 1,
                            width: l.size.width,
                            height: drawHeight
                        )
                    )
                    context.draw(
                        Image(nsImage: r),
                        in: CGRect(
                            x: topRight.x - tr.size.width + edgeSizes.right,
                            y: topRight.y + tr.size.height - edgeSizes.top - 1,
                            width: r.size.width,
                            height: drawHeight
                        )
                    )
                    ...
                    context.draw(
                        Image(nsImage: tr),
                        at: CGPoint(
                            x: topRight.x + edgeSizes.right,
                            y: topRight.y - edgeSizes.top
                        ),
                        anchor: .topTrailing
                    )
                    ...
                    context.draw(
                        Image(nsImage: bl),
                        at: CGPoint(
                            x: bottomLeft.x - edgeSizes.left,
                            y: bottomLeft.y + edgeSizes.bottom
                        ),
                    ...
                    context.draw(
                        Image(nsImage: br),
                        at: CGPoint(
                            x: bottomRight.x + edgeSizes.right,
                            y: bottomRight.y + edgeSizes.bottom
                        ),
                    ...
                    context.draw(
                        ...
                        in: CGRect(
                            x: center.x - screenshotSize.width / 2,
                            y: center.y - screenshotSize.height / 2,
    ...
    func draw( // swiftlint:disable:this function_body_length
        inputs: [SimulatorImageInput],
        in context: GraphicsContext,
        canvasSize size: CGSize,
        screenshotSize: CGSize,
        edges: SimulatorImagePadding
    ) {
        for input in inputs {
            ...
            var y =
                (size.height - screenshotSize.height) / 2.0
                + Double(input.offsets.normal.y) + image.size.height / 2
                - edges.top
            // swiftlint:enable identifier_name
            ...
            case "left":
                ...
                x =
                    (size.width - screenshotSize.width) / 2.0
                    - input.offsets.normal.x - edges.left
                    + (chrome.images.devicePadding.left / 2.0)
                anchor = .leading
            ...
            case "right":
                ...
                x =
                    (size.width + screenshotSize.width) / 2.0
                    + input.offsets.normal.x + edges.left
                    - (chrome.images.devicePadding.right / 2.0)
                anchor = .leading
            ...
            case "bottom":
                ...
                y =
                    (size.height - screenshotSize.height) / 2.0
                    + (screenshotSize.height) + abs(input.offsets.normal.y)
                anchor = .bottom
            ...
            case "top":
                ...
                if input.align == "trailing" {
                    x =
                        (size.width + screenshotSize.width) / 2.0
                        + input.offsets.normal.x
                        + chrome.images.devicePadding.right
                } else {
                ...
                y =
                    (size.height - screenshotSize.height) / 2.0
                    - input.offsets.normal.y - chrome.images.devicePadding.top
                    - image.size.height
                anchor = .bottom

            default:
                print(
                    "IMPORTANT: Unknown anchor for simulator image \(input.image)"
                )
                x = 0
                anchor = .center
            }
